...
<script>
    export default {
        ...
        methods: {
            ...
            getPosts(page = 1){
                axios.get(this.$http + 'posts/advantages?page=' + page).then(({data}) => (this.posts = data));
            },
            createPost() {
                this.$Progress.start();
                const data = new FormData();
                ...
                axios.post(this.$http + 'posts/advantages', data, {headers: {'Content-Type': 'multipart/form-data'}})
                    .then(() => {
                        Fire.$emit('afterCreate');
                        $('#addNew').modal('hide');
                        Toast.fire({
